...
- **`ModuleNotFoundError` for any package at launch**: backtalk's Python packages never finished installing, or the environment drifted. From the backtalk folder run `uv sync`, which installs everything from the shipped package list, then launch again. (Launchers updated after this was found in the field run the repair automatically; if yours predates that, update backtalk.) If `uv sync` itself errors, run `uv venv .venv` then `uv pip install -e .` instead. On Windows certificate errors, use `uv --system-certs sync --inexact`.
- **Windows reaches `voice ready` or `speaking`, but no sound plays and shutdown hangs**: NumPy 2.5.3 can stall PortAudio playback on Windows audio devices. Backtalk pins Windows/Python 3.12 to the verified NumPy 2.5.2 release; run `uv --system-certs sync --inexact` from this folder to apply the pin, then relaunch. Python 3.11 stays on its compatible NumPy 2.4 line.
- **The greeting speaks, then it goes idle and ignores the key (open mic too)**: the step right after the greeting is connecting to Codex, the brain, and that connection failed or hung. It is the one startup step that needs a signed-in Codex installation, internet, and available plan usage. The voice line now says this failure out loud and the window stays open with the error; the ladder to fix it: run `codex` in a terminal and confirm a session opens signed in, check your internet, and check your plan has usage left. The log agrees: `logs/backtalk.log` ending at "connecting the brain..." with no "brain warm" after it is exactly this failure. Your browser choice has nothing to do with it; the visualizer only displays.
...
- **It starts cold and forgets the last conversation after a restart**: that is the default (a fresh session every launch is predictable). Want it to pick up where it left off? Tell your agent to set `"resume_last_session": true` in backtalk.json. From then on every launch reattaches to the previous Codex thread, and a stale saved thread falls back to fresh with a log line instead of breaking the launch. A long conversation can be compacted while recent turns remain available. Start over any time by saying "clear the session."
...
hold key -> ears.record_held (sounddevice, 16kHz int16)
         ...
         -> brain.ask_stream (warm Codex App Server session,
